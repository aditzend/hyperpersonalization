...
FAISS se ha convertido en una herramienta clave para realizar búsquedas de similitud en grandes bases de datos de vectores, mejorando el rendimiento y la escalabilidad de aplicaciones de inteligencia artificial y \textit{machine learning} que dependen de la búsqueda de alta precisión y rapidez.
...
El valor resultante de la distancia coseno varía entre 0 y 2. Un valor de 0 indica que los vectores son completamente similares (idénticos en dirección); un valor de 1, que son perpendiculares (sin relación en términos de dirección); y un valor de 2, que apuntan en direcciones opuestas.
...
El código implementado puede consultarse en detalle en el repositorio\footnote{Disponible en: \url{https://github.com/aditzend/hyperpersonalization/blob/main/app/notebooks/1.ipynb}}. El listado completo figura en el Anexo técnico (p.~\pageref{anexo:primera-iteracion-redis-faiss}).

Como conclusión se extrae que, en condiciones de laboratorio, se puede acceder a la información propia de un usuario mediante búsqueda por similitud vectorial sin necesidad de un esquema estricto. Conviene distinguir aquí dos decisiones independientes que más adelante suelen confundirse: \emph{segmentar por tipo de registro} y \emph{filtrar por usuario}. Lo que este experimento muestra es que no hace falta separar la información en colecciones por tema, porque la similitud semántica recupera el dato correcto dentro de un mismo espacio vectorial.

No es necesario armar índices por tema ni ser estricto con el esquema: se puede extraer el vector deseado por similitud semántica. El filtrado por \texttt{person\_id}, en cambio, responde a otra necesidad —aislar los datos de un usuario de los de otros— y será imprescindible en un entorno productivo, como se aborda en la iteración siguiente.