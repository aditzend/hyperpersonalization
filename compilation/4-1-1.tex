...
El código implementado puede consultarse en detalle en el repositorio\footnote{\url{https://github.com/aditzend/hyperpersonalization/blob/main/app/notebooks/1.ipynb}}. El listado completo figura en el Anexo técnico (p.~\pageref{anexo:primera-iteracion-redis-faiss}).

Como conclusión se extrae que, en condiciones de laboratorio, se puede acceder a la información propia de un usuario mediante distancia coseno —el algoritmo por defecto del motor de búsqueda de la base vectorial FAISS— sin necesidad de un esquema estricto. Conviene distinguir aquí dos decisiones independientes que más adelante suelen confundirse: \emph{segmentar por tipo de registro} y \emph{filtrar por usuario}. Lo que este experimento muestra es que no hace falta separar la información en colecciones por tema, porque la similitud semántica recupera el dato correcto dentro de un mismo espacio vectorial.
