...
timestamp = datetime.now().isoformat()
message = {"timestamp": timestamp, 
           "content": "mis hijas se llaman Aurora y Lucía, Aurora tiene 12 y Lucía 9"} 
text = str(message)
db.add_texts(texts=[text], embedding=embeddings, metadatas=metadatas)
...
\subsection{Agregado de un paso de extracción y limpieza a un conjunto de datos con cuatro miembros de un grupo familiar}
...
\begin{APACode}[breaklines=true]
  ...
  churn_pessim  = 0.17       # 17 % (pesimista: punto medio de la banda Delphi 12-22 %)
  ...
  nc_pessim  = {1:2,  2:3,  3:5,  4:7,  5:9,  6:11, 7:13}
  ...
  # --- OPEX operativo ligado a la adopción ---
  # El equipo NO se dota por completo desde el Año 1: una parte es un nucleo
  # fijo (independiente de la cantidad de clientes) y otra escala con la base
  # de clientes activos. La misma regla se aplica a los tres escenarios.
  core_fixed_y1   = 78000    # nucleo fijo: CEO + liderazgo tecnico + admin parcial
                             #              + compliance (DLP/DPO/DR/pentest) + infra base
  opex_per_active = 1500     # soporte/monitoreo/operacion semivariable por cliente activo/ano
  admin_growth    = 0.04     # 4 % crecimiento anual del nucleo fijo (conservador)

  # Costo de inferencia LLM: se factura al cliente como pass-through
  # (ingreso = costo), referencia Sonnet ~USD 0.017/conversacion
  # (USD 3/MTok entrada, USD 15/MTok salida). Neutro en el FCFF, por lo que
  # no se incorpora a la ecuacion: cierra el costo de inferencia sin distorsionar el VAN.
  ...
  def run_scenario(new_customers, churn):
      ...
      for t in range(1, 8):
          ...
          fixed        = core_fixed_y1 * ((1 + admin_growth) ** (t - 1)) \
                        + opex_per_active * active[t]
      ...
      # FCFF (con traslado de quebrantos)
      fcff = {0: -capex[0]}
      nol = 0.0  # quebranto acumulado
      for t in range(1, 8):
