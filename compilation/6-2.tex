...
\begin{itemize}
    \item \textbf{Diferenciación}: Combina múltiples fuentes de datos y optimiza la hiperpersonalización en tiempo real, una integración que las soluciones actuales del mercado no ofrecen de forma combinada.
    \item \textbf{Privacidad y seguridad}: La instalación local o en un entorno controlado por el cliente reduce las preocupaciones sobre la transferencia de datos. El modelo de despliegue on-premise facilita el cumplimiento de la Ley~25.326 de Protección de Datos Personales y de las disposiciones de la Agencia de Acceso a la Información Pública (AAIP), dado que el repositorio de datos personales permanece dentro del perímetro del cliente y los controles de prevención de fuga de datos evitan el envío de información personal en claro a servicios externos de inferencia. Esto habilita la firma de acuerdos de tratamiento de datos (DPA) con roles claros de responsable y encargado, un requisito cada vez más frecuente en los procesos de compra de sectores regulados.
...
\begin{itemize}
    \item \textbf{Requiere personalización inicial}: Cada cliente puede necesitar adaptaciones específicas para integrar el \textit{middleware} con sus sistemas.
...
\begin{itemize}
    ...
    \item \textbf{Endurecimiento regulatorio}: La evolución de la normativa argentina de protección de datos personales —y la eventual convergencia con estándares internacionales como GDPR— podría elevar los requisitos de auditoría, consentimiento y tratamiento de datos de menores, incrementando los costos de compliance tanto para la empresa como para sus clientes. Este riesgo se mitiga parcialmente por la propia arquitectura on-premise de la solución, pero exige un seguimiento regulatorio activo.
\end{itemize}
...
Muchas empresas no pueden alojar o transmitir sus datos fuera de sus centros de datos propios por preocupaciones legales y de seguridad. En Argentina, la Ley~25.326 establece obligaciones de consentimiento informado, finalidad específica y medidas de seguridad para todo tratamiento de datos personales. En verticales como educación, donde los usuarios finales pueden ser menores de edad, se requiere además el consentimiento explícito de padres o tutores y políticas de no-persistencia de información personal identificable (PII) en los registros del sistema. La arquitectura on-premise del \textit{middleware} resuelve estas restricciones al mantener los datos dentro del perímetro del cliente, y la operación se complementa con acuerdos de tratamiento de datos (DPA) que definen roles, subencargados, plazos de retención y mecanismos de borrado.
...
\begin{itemize}
    ...
    \item \textbf{Cumplimiento normativo}: Los datos permanecen en el entorno seguro de la empresa, facilitando el cumplimiento de la Ley~25.326 y de regulaciones sectoriales (banca, salud, educación). Se proveen DPAs estándar y controles de prevención de fuga de datos (DLP) integrados al flujo de operación.
...
Aplicando la propuesta de valor a esta industria se recalca que el \textit{middleware} actúa como un intermediario entre los sistemas internos de la empresa y el motor del bot, permitiendo acceso contextualizado a los datos, integrando CRM, sistemas de reservas (e.g., Amadeus, Sabre), y registros de clientes frecuentes. Además la hiperpersonalización en tiempo real proporciona respuestas relevantes basadas en el historial del cliente, preferencias previas y datos actuales. Por el lado del cumplimiento de normativas, el aporte recae en que el repositorio de datos personales y la orquestación permanecen dentro del perímetro de la aerolínea; cuando la generación de respuestas se apoya en un modelo de lenguaje externo, los controles de prevención de fuga de datos (DLP) redactan la información personal identificable antes de cualquier envío, de modo que no se transfieren datos personales en claro fuera del perímetro.

\subparagraph{Escenario práctico: Cliente frecuente solicita información}

\begin{enumerate}
    \item Sin \textit{middleware}:
    \begin{itemize}
...
\begin{enumerate}
    ...
    \item Con \textit{middleware}:
    \begin{itemize}
        \item Bot: ``Hola, María. Tu cuenta principal tiene un saldo de USD 12.500. ¿Querés programar una transferencia o conocer los detalles de tu última transacción?''
        \item Usuario: ``Sí, necesito transferir USD 2.000.''
