...
En un entorno productivo, lo visto en el apartado anterior no es aplicable. Es necesario hacer una búsqueda por distancia coseno dentro de un espacio vectorial filtrado por \texttt{person\_id} de manera estricta. En la misma iteración se experimenta con la creación de una base vectorial en la que cada vector es un mensaje que el usuario tuvo con el asistente. En ese mensaje el usuario brinda una partícula de información importante que refuerza el nivel de conocimiento que el sistema tiene del usuario.
...
La transcripción íntegra de este listado figura en el \textit{Anexo técnico} (véase el Apéndice~\ref{anexo:code-4-1-2-1}).
...
La transcripción íntegra de este listado figura en el \textit{Anexo técnico} (véase el Apéndice~\ref{anexo:code-4-1-2-2}).
