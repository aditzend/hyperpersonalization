...
Se importan las librerías necesarias para generar embeddings y realizar búsquedas vectoriales utilizando FAISS, además de cargar Redis y las variables de entorno necesarias para configurar el sistema (Anexo técnico, p.~\pageref{anexo:code-4-1-3-1}).
...
Los listados completos figuran en el Anexo técnico (pp.~\pageref{anexo:code-4-1-3-2}, \pageref{anexo:code-4-1-3-3} y \pageref{anexo:code-4-1-3-4}).
...
El sistema devuelve el mensaje ``mi madre se llama Alicia, tiene 66, es jubilada'' con un puntaje de similitud (Anexo técnico, p.~\pageref{anexo:code-4-1-3-5}).

\textbf{Consulta 2: ``¿Tiene hijos?''}

Se devuelven dos mensajes: ``a mis hijas no les gusta estar en el pasillo, siempre ventana'' y ``mis hijas se llaman Aurora y Lucía, Aurora tiene 12 y Lucía 9'' (Anexo técnico, p.~\pageref{anexo:code-4-1-3-6}).

\textbf{Consulta 3: ``¿Cuáles son las preferencias de los hijos?''}

El sistema devuelve que las hijas prefieren sentarse junto a la ventana (Anexo técnico, p.~\pageref{anexo:code-4-1-3-7}).

\textbf{Consulta 4: ``¿Edad de la hija mayor?''}

El sistema devuelve el mensaje con la edad de Aurora, la hija mayor (Anexo técnico, p.~\pageref{anexo:code-4-1-3-8}).
